...
Vector3D::Vector3D( float aX, float aY, float aW ) noexcept: fBaseVector(Vector2D(aX,aY)),fW(aW) 
{
}
...
{
    return Vector3D{x()*aScalar,y()*aScalar,fW*aScalar};
}

Vector3D Vector3D::operator+( const Vector3D& aOther ) const noexcept
{
    return Vector3D{x()+aOther.x(),y()+aOther.y(),fW+aOther.fW};
}

float Vector3D::dot( const Vector3D& aOther ) const noexcept
{
    return x()*aOther.x() + y()*aOther.y() + fW*aOther.fW;
}
